...
CREATE TABLE parties (
    ...
    -- Уникальность, чтобы не создавать дубликатов людей
...
CREATE TABLE court_cases (
    ...
    close_date DATE,
    -- Используем ENUM для жесткой типизации исходов
...
CREATE TABLE documents (
    ...
    case_id INT NOT NULL,
    judge_id INT, -- Может быть NULL, если документ подан не судьей, а канцелярией или стороной
    ...
    FOREIGN KEY (case_id) REFERENCES court_cases(case_id) ON DELETE CASCADE,
    FOREIGN KEY (judge_id) REFERENCES judges(judge_id),
    -- Дата регистрации не может быть раньше даты создания документа
